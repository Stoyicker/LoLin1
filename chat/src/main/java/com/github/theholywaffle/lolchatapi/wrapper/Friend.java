...
public class Friend extends Wrapper<RosterEntry> implements Comparable {
     ...
     */
    @SuppressWarnings("unused") //Future will tell
     ...
     * division and league, queue name, game status,...)
     *
     * @return Status
     */
    @SuppressWarnings("unused")
    ...
    public boolean matchesFilterQuery(CharSequence constraint) {
        return getName().toLowerCase(Locale.ENGLISH).contains((constraint + "").toLowerCase
                (Locale.ENGLISH));
    }
    ...
    public int compareTo(@NonNull Object another) {
        if (!(another instanceof Friend))
            throw new IllegalArgumentException("Non-comparable classes " + Friend.class + " and "
                    + another.getClass());
        int thisValue, anotherValue;
        final ChatMode thisCM = getChatMode(), anotherCM = ((Friend) another).getChatMode();
